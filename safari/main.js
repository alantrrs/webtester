...
function load() {
    mraidview.addEventListener('info', function (message) {
        now = new Date();
        console.log([formatInfoTime(now), ' INFO ', message].join(''));
    });

    mraidview.addEventListener('error', function (message) {
        now = new Date();
        console.log([formatInfoTime(now), ' ERROR ', message].join(''));
    });
}
...
function renderHtmlAd(mraidType) {
  	var form = document.forms.setup;
    prepareMraidView(form);
    ...
    function loadXMLDoc() {
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                mraidview.setUseHtml(true, xmlhttp.responseText);
                mraidview.render();
            }
        }

        if (mraidType == "interstitial") {
            xmlhttp.open("GET", "/js/mraid/iab/interstitial.html", true);
        } else if (mraidType == "expandable") {
            xmlhttp.open("GET", "/js/mraid/iab/expandable.html", true);
        }
        xmlhttp.send();
    }
    loadXMLDoc();
